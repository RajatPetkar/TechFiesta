...
import { BarChart3, Users, Package, DollarSign, MoreVertical } from "lucide-react";
...
function Dash() {
  ...
  useEffect(() => {
    ...
    setHighestScore(allAttempts.length > 0 ? Math.max(...allAttempts.map(q => q.score)) : "N/A");

    // Calculate attempted quizzes count
    setAttemptedQuizzes(quizResults.length);

    setAverageScore(allAttempts.length > 0
      ? (allAttempts.reduce((sum, q) => sum + q.score, 0) / allAttempts.length).toFixed(2)
      : "N/A");
  ...
  const progressPercentage = quizResults.length > 0
      ...
      <main className="p-5">
        <div className="row g-4 mb-4">
          <div className="col-md-6 col-xl-3">
            <StatsCard title="Users" value={userCount} icon={Users} color="green" />
          </div>
          <div className="col-md-6 col-xl-3">
            <StatsCard title="Attempted Quiz" value={attemptedQuizzes} icon={Package} color="yellow" />
          </div>
          <div className="col-md-6 col-xl-3">
            <StatsCard title="Highest Score" value={highestScore + `/10`} icon={DollarSign} color="blue" />
          </div>
        </div>
            ...
            <div className="d-flex justify-content-between align-items-center">
              <div>
                ...
                <div className="col-md-5 col-xl-6 p-1 mb-2">
                  <StatsCard title="Progress" value={averageScore} icon={BarChart3} color="purple" />
                </div>
                <Button onClick={() => navigate("/reports")} variant="btn btn-dark m-1" >View Report</Button>
              </div>
              <div className="col-3">
        ...
        <Card className="mb-4 shadow-sm">
          <Card.Body>
            ...
            <div className="d-flex align-items-center gap-2 mb-3">
              <p className="text-muted small mb-0">Total 48.5% growth</p>
            ...
            <div className="row g-4 mt-3">
              <div className="col-md-6 col-xl-3">
                ...
                <ProgressBar now={progressPercentage} variant="success" className="rounded-pill" />
              </div>
            </div>
          </Card.Body>
        </Card>
              ...
              <Row>
                {quizResults.map((result, index) => (
                  <Col key={index} md={3} className="mb-4">
                    <Card>
                      <Card.Body>
                        <p className="text-muted small mb-1">Topic: {result.topic || "N/A"}</p>
                        <p className="text-muted small mb-1">Score: {result.score}</p>
                        <p className="text-muted small mb-1">
                          Date Taken: {new Date(result.dateTaken).toLocaleDateString()}
                        </p>
                        <Button variant="btn btn-dark" onClick={() => handleViewQuiz(result)}>
                          View
                        </Button>
                      </Card.Body>
                    </Card>
        ...
        <Modal show={showModal} onHide={handleCloseModal} centered>
          ...
          <Modal.Body>
            {selectedQuiz && (
              <>
                <p><strong>Topic:</strong> {selectedQuiz.topic || "N/A"}</p>
                <p><strong>Score:</strong> {selectedQuiz.score}</p>
                <p><strong>Date Taken:</strong> {new Date(selectedQuiz.dateTaken).toLocaleDateString()}</p>
                <p><strong>Questions:</strong></p>
                <ul>
                  {selectedQuiz.questionsAnswered.map((question, idx) => (
                    <li key={idx}>
                      <p>{idx + 1}) {question.questionText}</p>
